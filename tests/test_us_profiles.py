...
us_profiles_conn = AggregateProfilesConnection()


def test_get_status():
    message = us_profiles_conn.get_status()
...
def test_is_healthy():
    """
    """
    assert us_profiles_conn.is_healthy(), "connection is not healthy"


def test_metadata():
    """
    """
    response = us_profiles_conn.get_metadata()
...
def test_post_recency():
    ...
    response = us_profiles_conn.post_recency(payload)
...
def test_get_rankings():
    response = us_profiles_conn.get_rankings()
...
def test_post_rankings():
    ...
    response = us_profiles_conn.post_rankings(facet, payload)
...
def test_get_taxonomies():
    response = us_profiles_conn.get_taxonomies()
    assert len(response) != 0, "No taxonomies? Seems dubious"

    response = us_profiles_conn.get_taxonomies(facet = 'title', q = 'Data Scientist')
...
def test_post_taxonomies():
    ...
    response = us_profiles_conn.post_taxonomies('soc5', payload)
